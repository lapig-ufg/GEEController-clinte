...
class HelpLapig:
    ...
    def expression_select(self, image, satellite, band):
        return image.expression(self.getExpression(satellite, band)).select(
            [0], [band]
        )
    ...
    def getLatLong(self, img):
        ...
        return self.ee.Image(img).addBands(
            latlon.select('longitude', 'latitude')
        )
    ...
    def getSRTM(self):
        ...
        return {
            ...
            'slope': radians(terrain.select('slope')).expression(
                'b("slope")*100'
            ),
